...
## Недавние изменения (05.11.2025)

### Исправление обработки геопозиции для Tasks v2 (StaffProBot)
- Исправлены конфликты области видимости переменных при обработке геопозиции
- Все сценарии обработки геопозиции работают корректно: открытие/закрытие смены, завершение задач Tasks v2
- Добавлен глобальный импорт `User` и явное объявление `global` для корректной работы Python
- Файлы: `apps/bot/handlers_div/core_handlers.py`

### Множественная загрузка медиа для задач Tasks v2 (StaffProBot)
- Добавлена возможность загрузки до 5 файлов (фото/видео) при выполнении задач Tasks v2
- Улучшен Media Orchestrator: добавлены методы `get_collected_count()` и `can_add_more()` для проверки состояния потока
- Упрощен UX: пользователь отправляет файлы подряд, кнопка "✅ Готово" появляется после каждого файла
- Исправлена ошибка с `caption` в `InputMediaPhoto`/`InputMediaVideo` (caption передается в конструктор)
- Все файлы отправляются в группу отчетов одним медиа-группой или отдельными сообщениями (для смешанных типов)
- Поддержка множественных файлов также добавлена для подтверждающих документов при отмене смены

### Исправление шаблонов поддержки (StaffProBot)
- Исправлены шаблоны `/support/*` для корректного отображения контента для всех ролей (owner, manager, employee)
- Добавлены блоки `manager_content` и `content` в шаблоны support для совместимости с разными базовыми шаблонами
- Обновлена документация в `doc/vision_v1/roles/*` с указанием support-роутов
